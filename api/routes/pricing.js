...
const storage = multer.diskStorage({
  ...
  filename: (req, file, callBack) => {
    callBack(null, String(file.originalname).toLowerCase().split(" ").join("-"));
  }
});
...
router.get('/pricing', (req, res) => {
  Pricing.find({})
    ...
    .catch(err => res.status(404).json({error: true, errorMessage: err.message}))
})
...
router.post('/pricing', upload.single('image'), (req, res) => {
  ...
  pricing
    .save()
    .then(pricing => res.status(201).json({success: true, successMessage: 'Pricing Data Added Successfully', pricing}))
