...
const SortablePricingTable = ({editPricing, deletePricing}) => {
  ...
  return(
    <div className="sortable-pricing-table my-5 px-1" style={{
      height: '400px',
      overflowY: 'scroll'
    }}>
      <button
        ...
        className="btn btn-primary float-right">
          Add New
        <i className={pricing.length > 0 ? "fas fa-angle-up pl-1" : "fas fa-spinner fa-spin pl-1"}></i>
      </button>
      ...
      <table className="table table-hover" >
        ...
        <tbody>
          {pricing.length > 0 && pricing.map(item => (
            <tr key={item._id}>
              <th scope="row">{String(item._id).substr(0, 5).concat("...")}</th>
              <td>{item.name}</td>
              <td>{item.price}</td>
              <td>{item.discount}%</td>
              <td>
